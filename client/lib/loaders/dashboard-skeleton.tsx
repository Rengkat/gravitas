// components/ui/loaders/dashboard-skeleton.tsx
//
// Skeleton placeholder screen for the student dashboard.
// Server Component — no client directive needed (pure CSS shimmer).
//
// Usage:
...
/* ── Shared shimmer bone ── */
function Bone({ w, h, r = 8, delay, className = '' }: {
  w?: number | string; h: number; r?: number; delay?: string; className?: string
}) {
  return (
    <div
      className={className}
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ height: 72, background: 'white', borderBottom: '1px solid rgba(26,74,46,0.08)', padding: '0 5%' }}
      >
        <Bone w={38} h={38} r={10} />
        <Bone w={90} h={18} r={5}  />
        <div className="flex gap-2 ml-5">
          {[80, 94, 68, 72].map((w, i) => <Bone key={i} w={w} h={13} r={3} />)}
        </div>
      ...
      {/* ── Body ── */}
      <div className="flex flex-1 min-h-0" style={{ display: 'grid', gridTemplateColumns: '240px 1fr' }}>
        ...
        >
          ...
          {SIDEBAR_ITEMS.map((opacity, i) => (
            <div key={i} className="flex items-center gap-2.5 px-3 py-2.5 rounded-[10px]" style={{ opacity }}>
              <Bone w={20} h={20} r={6} />
              <Bone h={13} r={4} />
            </div>
          ))}
        ...
        <div style={{ padding: '32px 40px', overflow: 'hidden' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 16, marginBottom: 28 }}>
            {STAT_WIDTHS.map((w, i) => (
              <div key={i} style={{ padding: 20, borderRadius: 16, background: 'white', border: '1px solid rgba(26,74,46,0.08)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
            {/* Exam list */}
            <div style={{ padding: 24, borderRadius: 16, background: 'white', border: '1px solid rgba(26,74,46,0.08)' }}>
            ...
            <div style={{ padding: 24, borderRadius: 16, background: 'white', border: '1px solid rgba(26,74,46,0.08)' }}>
              ...
              <div className="flex justify-between">
                {BAR_HEIGHTS.map((_, i) => <Bone key={i} w={24} h={10} r={3} />)}
              </div>
            </div>
          </div>
        </div>
