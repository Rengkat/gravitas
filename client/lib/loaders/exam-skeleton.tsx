// components/ui/loaders/exam-skeleton.tsx
//
// Skeleton placeholder screen for the live CBT exam interface.
// Server Component — no client directive needed.
//
// Usage:
//   import ExamSkeleton from '@/components/ui/loaders/exam-skeleton'
//
//   // In app/(dashboard)/practice/session/[sessionId]/loading.tsx:
//   export default function Loading() { return <ExamSkeleton /> }

/* ── Shimmer bone ── */
function Bone({
  w, h, r = 8, delay, bg, className = '', style: extraStyle,
}: {
  w?: number | string; h: number; r?: number; delay?: string
  ...
  return (
    <div
      ...
      style={{
        ...
        height:       h,
        borderRadius: r,
        background:   bg ?? 'linear-gradient(90deg, rgba(26,74,46,0.06) 25%, rgba(26,74,46,0.11) 50%, rgba(26,74,46,0.06) 75%)',
...
const CELL_STATES: ('answered' | 'flagged' | 'current' | 'empty')[] = [
  ...
  'answered','answered','answered','answered','answered','current',
  ...Array(28).fill('empty'),
]
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex gap-1.5">
          {TAB_WIDTHS.map((w, i) => (
            <div
              key={i}
              style={{
                width: w, height: 30, borderRadius: 6,
        ...
        {/* Timer placeholder */}
        <div style={{ width: 110, height: 34, borderRadius: 8, background: 'rgba(230,57,70,0.5)', animation: 'sk-shimmer-dark 1.6s ease-in-out infinite' }} />
      </div>
      ...
      <div className="flex flex-1 min-h-0">
        ...
        <div style={{ flex: 1, padding: '32px 40px', overflow: 'hidden' }}>
          ...
          <div className="mt-6 p-4 rounded-xl" style={{ background: 'rgba(13,43,26,0.85)' }}>
            <div style={{ width: 110, height: 11, borderRadius: 3, background: 'rgba(245,200,66,0.3)', marginBottom: 12 }} />
            <div style={{ height: 13, borderRadius: 4, background: 'rgba(255,255,255,0.1)', marginBottom: 8 }} />
            <div style={{ height: 13, width: '75%', borderRadius: 4, background: 'rgba(255,255,255,0.07)' }} />
          </div>
        </div>
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    ...(isEmpty ? {
                      background: 'linear-gradient(90deg, rgba(26,74,46,0.06) 25%, rgba(26,74,46,0.11) 50%, rgba(26,74,46,0.06) 75%)',
                      backgroundSize: '200% 100%',
