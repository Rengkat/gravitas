...
import {
  ...
  Check, Lock, Shield, CreditCard, Phone,
  GraduationCap, Users, BookOpen, School,
  PenSquare, Briefcase, Target, ClipboardList,
  FileText, Stethoscope,
} from "lucide-react";
import type { LucideIcon } from "lucide-react";
import type { RoleId } from "./Step1RoleSelect";
...
const ROLE_CONFIG: Record<RoleId, RoleConfig> = {
  student: {
    Icon: GraduationCap, label: "Student Account",
    heading: "Create your student account",
    sub: "Start preparing for JAMB, WAEC, and more.",
    showExamTarget: true, showSchoolName: false, showSubject: false, showLGA: false,
  },
  parent: {
    Icon: Users, label: "Parent / Guardian Account",
    heading: "Create your parent account",
    sub: "Monitor your child's progress and receive weekly WhatsApp reports.",
    showExamTarget: false, showSchoolName: false, showSubject: false, showLGA: false,
  },
  tutor: {
    Icon: BookOpen, label: "Tutor Account",
    heading: "Apply as a Gravitas tutor",
    sub: "Teach students online or near your location. Earn per session.",
    showExamTarget: false, showSchoolName: false, showSubject: true, showLGA: true,
  },
  school: {
    Icon: School, label: "School / Institution Account",
    heading: "Set up your school portal",
    sub: "Get a branded portal with CBT test builder, auto-grading, and parent reports.",
    showExamTarget: false, showSchoolName: true, showSubject: false, showLGA: false,
  },
  teacher: {
    Icon: PenSquare, label: "Teacher Account",
    heading: "Create your teacher account",
    sub: "Upload materials, set tests, and track your students' performance.",
    showExamTarget: false, showSchoolName: false, showSubject: true, showLGA: false,
  },
  professional: {
    Icon: Briefcase, label: "Professional Account",
...
/* ─── Exam targets ───────────────────────────────────────────────────────────
   Each target is its own branch in ExamTargetButton — fully static classes.
──────────────────────────────────────────────────────────────────────────── */
type ExamTarget = { value: string; label: string; Icon: LucideIcon };
...
const STATES = [
  "Lagos","Abuja (FCT)","Rivers","Kano","Oyo","Kaduna","Anambra","Delta","Enugu",
  "Ogun","Imo","Kwara","Osun","Ekiti","Ondo","Cross River","Akwa Ibom","Edo",
  "Benue","Plateau","Niger","Kogi","Nassarawa","Taraba","Adamawa","Gombe","Bauchi",
  "Borno","Yobe","Jigawa","Katsina","Kebbi","Sokoto","Zamfara","Bayelsa","Abia","Ebonyi",
];
...
function getStrength(val: string): { score: number; label: string } {
  ...
  const labels = ["", "Weak — add uppercase & numbers", "Fair — add a special character", "Good — almost there!", "Strong ✓"];
...
function FieldLabel({ text, hint }: { text: string; hint?: string }) {
  return (
    <label className="block text-xs font-semibold text-green-900/70 mb-1.5 tracking-wide uppercase">
      {text}
      {hint && <span className="text-green-700/40 normal-case font-normal tracking-normal ml-1">{hint}</span>}
    </label>
  );
}
...
export default function Step2Details({ role, onBack, onSubmit }: Props) {
  ...
  const { score: pwScore, label: pwLabel } = getStrength(password);

  /* ── Strength bar colours — static literals, indexed by score ── */
  const strengthColors = ["bg-green-900/10", "bg-red-400", "bg-orange-400", "bg-yellow-400", "bg-green-500"];

  function validate() {
    const errs: Record<string, string> = {};
    if (firstName.length < 2) errs.firstName = "Too short";
    if (lastName.length < 2)  errs.lastName  = "Too short";
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) errs.email = "Enter a valid email address";
    if (password.length < 8)  errs.password  = "Password must be at least 8 characters";
    if (confirm !== password)  errs.confirm   = "Passwords do not match";
    if (!agreeTerms)           errs.terms     = "Please agree to the Terms of Service";
    return errs;
  }
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    setTimeout(() => { setLoading(false); onSubmit(email); }, 1800);
  }
  ...
  return (
    <div className="[animation:fadeUp_0.4s_ease_both]">
      ...
      <div className="mb-8">
        ...
        {/* Role badge — uses the same LucideIcon as Step1 */}
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <span className="text-xs text-green-700/40 font-semibold tracking-widest uppercase">or with email</span>
        <div className="flex-1 h-px bg-green-900/10" />
      </div>
      ...
      <form onSubmit={handleSubmit} noValidate>
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          <div>
            ...
            <input type="text" value={firstName} onChange={e => { setFirstName(e.target.value); setErrors(p => ({ ...p, firstName: "" })); }}
              placeholder="Adaeze" className={inputCls("firstName")} />
          ...
          <div>
            ...
            <input type="text" value={lastName} onChange={e => { setLastName(e.target.value); setErrors(p => ({ ...p, lastName: "" })); }}
              placeholder="Okonkwo" className={inputCls("lastName")} />
        ...
        {cfg.showSchoolName && (
          <div className="mb-4">
            ...
            <input type="text" value={schoolName} onChange={e => setSchoolName(e.target.value)}
              placeholder="Kings College Lagos" className={inputCls("schoolName")} />
          </div>
        )}
        ...
        <div className="mb-4">
          ...
          <div className="relative">
            <input type="email" value={email}
              onChange={e => { setEmail(e.target.value); setEmailValid(/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(e.target.value)); setErrors(p => ({ ...p, email: "" })); }}
              placeholder="adaeze@example.com" className={`${inputCls("email")} pr-10`} />
            {emailValid && (
        ...
        {cfg.showSubject && (
          <div className="mb-4">
            ...
            <select title="subject" value={subject} onChange={e => setSubject(e.target.value)}
              ...
              <optgroup label="Sciences">
                ...
                <option>Biology</option><option>Further Mathematics</option>
              </optgroup>
              <optgroup label="Arts & Social Sciences">
                <option>English Language</option><option>Literature</option>
                <option>Government</option><option>Economics</option><option>Geography</option>
              </optgroup>
              <optgroup label="Commercial">
                <option>Commerce</option><option>Accounting</option>
              </optgroup>
              <optgroup label="Professional">
                <option>ICAN</option><option>Nursing</option>
              </optgroup>
            </select>
          </div>
        )}
        ...
        {cfg.showLGA && (
          <div className="mb-4">
            ...
            <select title="select lga" value={lga} onChange={e => setLga(e.target.value)}
              className={`${inputCls("lga")} appearance-none cursor-pointer`}>
            ...
            <div className="grid grid-cols-3 gap-2">
              {EXAM_TARGETS.map(({ value, label, Icon: ExamIcon }) => (
                <label key={value}
                  ...
                  <input type="radio" name="examTarget" value={value}
                    checked={examTarget === value} onChange={() => setExamTarget(value)} className="sr-only" />
        ...
        <div className="mb-2">
          ...
          <div className="relative">
            <input type={showPw ? "text" : "password"} value={password}
              ...
              placeholder="Minimum 8 characters" className={`${inputCls("password")} pr-12`} />
        ...
        <div className="mb-4">
          <div className="flex gap-1.5 mb-1.5">
            {[0,1,2,3].map(i => (
              <div key={i} className={`h-1 flex-1 rounded-full transition-all duration-400 ${i < pwScore ? strengthColors[pwScore] : "bg-green-900/10"}`} />
            ))}
          </div>
        ...
        <div className="mb-5">
          ...
          <div className="relative">
            <input type={showConfirm ? "text" : "password"} value={confirm}
              ...
              placeholder="Re-enter your password" className={`${inputCls("confirm")} pr-12`} />
        ...
        <div className="space-y-3 mb-6">
          <label className="flex items-start gap-3 cursor-pointer">
            <input type="checkbox" checked={agreeTerms} onChange={e => setAgreeTerms(e.target.checked)} className="g-checkbox mt-0.5" />
            <span className="text-[13px] text-green-900/60 leading-relaxed">
              I agree to Gravitas&apos;s{" "}
              <a href="#" className="text-green-700 font-semibold hover:text-green-800 underline underline-offset-2">Terms of Service</a>{" "}
              and{" "}
              <a href="#" className="text-green-700 font-semibold hover:text-green-800 underline underline-offset-2">Privacy Policy</a>
            </span>
          </label>
          {errors.terms && <p className="text-xs text-red-500 ml-0.5">{errors.terms}</p>}
          <label className="flex items-start gap-3 cursor-pointer">
            <input type="checkbox" checked={agreeMarketing} onChange={e => setAgreeMarketing(e.target.checked)} className="g-checkbox mt-0.5" />
            <span className="text-[13px] text-green-900/60 leading-relaxed">
