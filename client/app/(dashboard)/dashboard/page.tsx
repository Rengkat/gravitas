...
const RECENT_SESSIONS: RecentSession[] = [
  { id: "1", examType: "JAMB", subject: "Mathematics", score: 28, total: 40, date: "Today", duration: "35 min" },
  { id: "2", examType: "JAMB", subject: "English", score: 32, total: 40, date: "Yesterday", duration: "32 min" },
  { id: "3", examType: "WAEC", subject: "Physics", score: 22, total: 30, date: "2 days ago", duration: "28 min" },
];
...
const UPCOMING_BOOKINGS: UpcomingBooking[] = [
  { id: "1", tutorName: "Dr. Adeola Williams", subject: "Mathematics", date: "Tomorrow", time: "4:00 PM" },
];
...
function StatCard({ icon: Icon, label, value, color }: { icon: any; label: string; value: string; color: string }) {
  return (
    <div className="p-5 rounded-2xl bg-white border transition-all duration-300 hover:-translate-y-1" style={{ borderColor: "rgba(30,80,50,0.08)" }}>
  ...
  return (
    <div className="p-4 rounded-xl bg-white border" style={{ borderColor: "rgba(30,80,50,0.08)" }}>
      <div className="flex items-start justify-between mb-2">
        ...
        <div className="px-2 py-1 rounded-full text-[10px] font-bold" style={{ background: `${priorityColor}15`, color: priorityColor }}>
      ...
      <div className="mt-3">
        <div className="flex justify-between text-[11px] mb-1">
          ...
          <span className="font-semibold" style={{ color: accuracy < 50 ? "#ef4444" : "#f59e0b" }}>{accuracy}%</span>
        </div>
        <div className="h-1.5 rounded-full bg-cream overflow-hidden">
          <div className="h-full rounded-full transition-all" style={{ width: `${accuracy}%`, background: accuracy < 50 ? "#ef4444" : "#f59e0b" }} />
        </div>
      </div>
      <button className="w-full mt-3 py-2 rounded-lg text-[12px] font-semibold transition-all" style={{ background: "rgba(26,74,46,0.08)", color: "#1a4a2e" }}>
        Practice Topic →
      </button>
    </div>
  );
  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <p className="text-text-muted">Your exam is in 45 days. Let's make every session count.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <Link href="/dashboard/practice" className="group p-5 rounded-2xl bg-gradient-to-r from-green-800 to-green-700 text-white transition-all hover:scale-[1.02]">
          <div className="flex items-center justify-between mb-3">
            <Target size={24} />
            <ArrowRight size={18} className="opacity-0 group-hover:opacity-100 transition-all group-hover:translate-x-1" />
          </div>
        ...
        <Link href="/dashboard/ai-tutor" className="group p-5 rounded-2xl bg-gradient-to-r from-gold to-gold-dark text-green-900 transition-all hover:scale-[1.02]">
          <div className="flex items-center justify-between mb-3">
            <Zap size={24} />
            <ArrowRight size={18} className="opacity-0 group-hover:opacity-100 transition-all group-hover:translate-x-1" />
          </div>
        ...
        <Link href="/dashboard/games" className="group p-5 rounded-2xl bg-gradient-to-r from-purple-600 to-purple-500 text-white transition-all hover:scale-[1.02]">
          <div className="flex items-center justify-between mb-3">
            <Award size={24} />
            <ArrowRight size={18} className="opacity-0 group-hover:opacity-100 transition-all group-hover:translate-x-1" />
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="p-6 rounded-2xl bg-white border" style={{ borderColor: "rgba(30,80,50,0.08)" }}>
            <div className="flex items-center justify-between mb-4">
              ...
              <Link href="/dashboard/reports" className="text-[12px] font-semibold text-green-600 hover:text-green-700">View All →</Link>
            </div>
            <div className="space-y-3">
              {RECENT_SESSIONS.map((session) => (
                <Link key={session.id} href={`/dashboard/reports/${session.id}`} className="flex items-center justify-between p-3 rounded-xl hover:bg-cream transition-colors">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-[11px] text-text-muted">{session.examType} • {session.date} • {session.duration}</div>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="text-[15px] font-bold text-green-900">{session.score}/{session.total}</div>
                    <div className="text-[11px] text-text-muted">{Math.round((session.score / session.total) * 100)}%</div>
                  </div>
                </Link>
              ))}
            </div>
        ...
        <div className="space-y-6">
            ...
            {UPCOMING_BOOKINGS.length > 0 ? (
              UPCOMING_BOOKINGS.map((booking) => (
                <div key={booking.id} className="p-3 rounded-xl bg-cream">
                  ...
                  <div className="text-[12px] text-text-muted mb-2">{booking.subject} • {booking.date} at {booking.time}</div>
                  <Link href={`/dashboard/bookings/${booking.id}/session`} className="inline-flex items-center gap-1 text-[12px] font-semibold text-green-600">
              ...
              <div className="text-center py-6">
                ...
                <Link href="/dashboard/tutors" className="inline-block mt-2 text-[12px] font-semibold text-green-600">Book a tutor →</Link>
              </div>
            )}
          </div>
          ...
          <div className="p-6 rounded-2xl bg-gradient-to-br from-green-800 to-green-700 text-white">
            ...
            <Link href="/dashboard/games" className="inline-flex items-center gap-1 mt-4 text-[12px] font-semibold text-gold">
