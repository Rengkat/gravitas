  ...
  return (
    <aside className="flex flex-col gap-6">
      ...
      <div className="rounded-[20px] bg-green-800 p-6 relative overflow-hidden">
        ...
        <div className="absolute top-0 right-0 w-40 h-40 rounded-full pointer-events-none opacity-20" style={{
        ...
        <div className="relative z-[1]">
          ...
          <p className="text-white/30 text-[11px] mt-3 text-center">No spam. Unsubscribe anytime.</p>
        </div>
      </div>
      ...
      <div className="rounded-[20px] bg-white border border-cream-border p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-sm font-bold text-green-900 uppercase tracking-[0.08em]">Popular Topics</h3>
        </div>
        <div className="flex flex-wrap gap-2">
          {POPULAR_TAGS.map(tag => (
            <a key={tag} href={`/blog?q=${encodeURIComponent(tag)}`}
      ...
      {/* ── Popular Posts ── */}
      <div className="rounded-[20px] bg-white border border-cream-border p-6">
        <h3 className="text-sm font-bold text-green-900 uppercase tracking-[0.08em] mb-4">Most Read</h3>
          ...
          {popular.map((post, i) => (
            <a key={post.slug} href={`/blog/${post.slug}`}
              ...
              <div>
                ...
                <p className="text-[11px] text-text-light mt-1">{post.readTime} · {post.date}</p>
              </div>
            </a>
          ))}
        </div>
